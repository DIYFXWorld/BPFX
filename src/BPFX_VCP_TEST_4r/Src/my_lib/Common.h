...
#define	MS_2_LENGTH( ms )		((ms)*(Audio::Sampling_Rate)/1000)
#define	LENGTH_2_MS( len )	(1000*(len)/(Audio::Sampling_Rate))
...
template <typename T>
inline void Circuler_Inc( T& v, T min, T max )
{
	++v;
	if( v > max )
		v = min;
...
template <typename T>
inline void Circuler_Dec( T& v, T min, T max )
{
	--v;
	if( v < min )
		v = max;
